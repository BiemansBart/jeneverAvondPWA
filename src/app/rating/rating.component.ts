...
export class RatingComponent implements OnInit {
  ...
  changeRating(i: number) {
    if (this.localStorage.get(this.jeneverNaam + '-timestamp') === null) {
      this.gebruikersRating = i + 1;
      console.log('uw rating bedraagt : ' + this.gebruikersRating);
      this.localStorage.set(this.jeneverNaam + '-timestamp', new Date().getTime());
      this.ratingEvent.emit(this.gebruikersRating);
    } else {
      if (new Date(this.localStorage.get(this.jeneverNaam + '-timestamp')).getTime() - new Date().getTime() >= 300000) {
        this.gebruikersRating = i + 1;
        console.log('uw rating bedraagt : ' + this.gebruikersRating);
        this.localStorage.set(this.jeneverNaam + '-timestamp', new Date().getTime());
        this.ratingEvent.emit(this.gebruikersRating);
